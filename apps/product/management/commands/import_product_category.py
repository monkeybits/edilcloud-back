...
from apps.product.models import Category, Typology
...
class Command(BaseCommand):
    ...
    def import_category(self, filename, sheet_index, logger):
        ...
        if (row[0].value.lower() != 'tipologia' or row[1].value.lower() != 'codice' or row[2].value.lower() != 'nome' or
            row[3].value.lower() != 'descrizione'):
            logger.info('Struttura errata del file. Le colonne corrette sono Tipologia, Codice, Nome, Descrizione')
            return
        ...
        for index in range(1, xl_sheet.nrows):
            try:
                ...
                if row[0].ctype == 0 or row[1].ctype == 0 or row[2].ctype == 0:
                    ...
                    typology = Typology.objects.get(code=row[0].value)
