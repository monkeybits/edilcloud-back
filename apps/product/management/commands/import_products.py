...
from apps.product.models import Subcategory, Unit, Product
...
class Command(BaseCommand):
    ...
    def import_category(self, filename, sheet_index, logger):
        ...
        if (row[0].value.lower() != 'sottocategoria' or row[1].value.lower() != 'codice' or row[2].value.lower() != 'nome' or
            row[3].value.lower() != 'descrizione' or row[4].value.lower() != 'unit' or row[5].value.lower() != 'prezzo'):
            logger.info('Struttura errata del file. Le colonne corrette sono Sottocategoria, Codice, Nome, Descrizione, Unit, Prezzo')
            return
        ...
        for index in range(1, xl_sheet.nrows):
            try:
                ...
                if (row[0].ctype == xlrd.XL_CELL_EMPTY or row[1].ctype == xlrd.XL_CELL_EMPTY or
                            row[2].ctype == xlrd.XL_CELL_EMPTY):
                     error_list.append('riga {}: contiene valori vuoti'.format(index))
                else:
                    subcategory = Subcategory.objects.get(code=row[0].value)
                    unit = None
                    price = 0
                    if row[4].ctype != xlrd.XL_CELL_EMPTY:
                        unit = Unit.objects.get(code=row[4].value)
                    if row[5].ctype != xlrd.XL_CELL_EMPTY:
